            ...
            <div class="category mb-10">
                ...
                <div class="frhoverinfo">
                    <ul class="productaction">
                        <li><a @click.prevent="toCart(product.price, product.stock)"><i class="fa fa-shopping-bag"> {{quantity}}</i></a></li>
                        <li><a @click.prevent="addQuantity"><i class="fa fa-plus"></i> </a> </li>
                        <li><a @click.prevent="minQuantity"><i class="fa fa-minus"></i></a></li>
                    </ul>
                </div>
                <div class="frproductinner innerposition">
                    <h4><a >{{product.name}}</a></h4>
                    <h5>Stock left: {{product.stock}}</h5>
...
export default {
  ...
  created () {
    ...
    this.$store.dispatch('getProduct', id)
      .then(({ data }) => {
        this.$store.commit('setProductName', data.name)
        this.$store.commit('setProductImage', data.image_url)
        this.$store.commit('setProductPrice', data.price)
        this.$store.commit('setProductStock', data.stock)
      })
      .catch(err => this.$alert(err))
